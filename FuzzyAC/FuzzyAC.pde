...
public void createFuzzyEngine(){
    ...
    // INPUT 1 — Room Temperature
    inputVariable1 = new InputVariable();
    inputVariable1.setName("RoomTemp");
    inputVariable1.setRange(0.0, 40.0);
    
    inputVariable1.addTerm(new Trapezoid("Freezing", 0, 0, 4, 8));
    inputVariable1.addTerm(new Triangle("Cold", 4, 8, 16));
    inputVariable1.addTerm(new Triangle("Warm", 12, 20, 28));
    inputVariable1.addTerm(new Triangle("Hot", 24, 32, 36));
    inputVariable1.addTerm(new Trapezoid("VeryHot", 32, 36, 40, 40));

    
        // INPUT 2 — Target Temperature
    inputVariable2 = new InputVariable();
    inputVariable2.setName("TargetTemp");
    inputVariable2.setRange(0.0, 40.0);
    
    inputVariable2.addTerm(new Trapezoid("Freezing", 0, 0, 4, 8));
    inputVariable2.addTerm(new Triangle("Cold", 4, 8, 16));
    inputVariable2.addTerm(new Triangle("Warm", 12, 20, 28));
    inputVariable2.addTerm(new Triangle("Hot", 24, 32, 36));
    inputVariable2.addTerm(new Trapezoid("VeryHot", 32, 36, 40, 40));

    // OUTPUT — AC Command
    outputVariable = new OutputVariable();
    outputVariable.setName("Command");
    outputVariable.setRange(-10.0, 10.0);
    
    
    outputVariable.setAggregation(new Maximum());
    outputVariable.setDefuzzifier(new Centroid(100));

    outputVariable.addTerm(new Trapezoid("MaxCool", -10, -10, -7, -5));
    outputVariable.addTerm(new Triangle("Cool", -7, -5, 0));
    outputVariable.addTerm(new Triangle("Stable", -2, 0, 2));
    outputVariable.addTerm(new Triangle("Heat", 0, 5, 7));
    outputVariable.addTerm(new Trapezoid("MaxHeat", 5, 7, 10, 10));
    ...
    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Freezing and TargetTemp is Warm then Command is MaxHeat", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Cold and TargetTemp is Warm then Command is Heat", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Cold and TargetTemp is Hot then Command is MaxHeat", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Warm and TargetTemp is Hot then Command is Heat", engine));

    // --- Cooling Rules ---
    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Hot and TargetTemp is Warm then Command is Cool", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is VeryHot and TargetTemp is Warm then Command is MaxCool", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Hot and TargetTemp is Cold then Command is MaxCool", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Warm and TargetTemp is Cold then Command is Cool", engine));

    // --- Stable Rules ---
    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Warm and TargetTemp is Warm then Command is Stable", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Cold and TargetTemp is Cold then Command is Stable", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Hot and TargetTemp is Hot then Command is Stable", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is Freezing and TargetTemp is Freezing then Command is Stable", engine));

    ruleBlock.addRule(Rule.parse(
        "if RoomTemp is VeryHot and TargetTemp is VeryHot then Command is Stable", engine));
